...
Two PostgreSQL **session guards** are always active, without any extra env var: `statement_timeout = 30s` (raised to `300s` only for the migration runner, which may run long DDL) caps any single query, and `idle_in_transaction_session_timeout = 30s` — per PostgreSQL docs, an idle-in-transaction session is terminated with `25P03` — limits how long a `BEGIN`…`COMMIT` may sit idle, reducing the window where a stuck `await` could hold row locks or bloat `pg_stat_activity`. Legitimate transactions are the short `beginStep`/`settleStep` and `claim`/`requeueExpired` bookkeeping the engine already runs outside handlers (the handler itself executes with no DB transaction held, per the two-transaction model), so `30s` is generous for normal work but tight enough to reclaim a leaked transaction. The value is centralized in [`src/db/client.ts`](src/db/client.ts) as `DEFAULT_IDLE_IN_TRANSACTION_TIMEOUT_MS` and sent as a `pg` startup parameter: verified locally via `SHOW` on a real PostgreSQL session and inside `BEGIN`/`COMMIT` (tests verify the GUC is active, not a 30s sleep and not the `25P03` abort itself); Neon honors the same startup parameter per wire-protocol docs, but end-to-end Neon behavior is only exercised when `TEST_DATABASE_URL` points at Neon.
