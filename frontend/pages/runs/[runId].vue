...
import { ArrowLeft, Bot, Clock3, Database, Wrench, Workflow, Sparkles, Activity } from '@lucide/vue';
...
<template>
  <div>
    ...
    <template v-else-if="inspection">
      <section class="panel panel--elevated run-hero" style="position:relative;overflow:hidden">
        <div aria-hidden="true" style="position:absolute;inset:0;background:radial-gradient(520px 220px at 85% 15%, rgba(47,111,237,0.07), transparent 60%), radial-gradient(420px 200px at 10% 90%, rgba(183,164,251,0.06), transparent 60%);pointer-events:none" />
        <div style="min-width:0;position:relative">
          <p class="eyebrow" style="display:flex;align-items:center;gap:8px"><Workflow :size="12" aria-hidden="true" />{{ inspection.workflow.name }} · v{{ inspection.version.version }} · <span class="pill" style="height:20px;font-size:11px;border-radius:999px">{{ inspection.version.triggerType }} trigger</span></p>
          <h2 :title="inspection.run.id" style="font-family:var(--font-mono);font-size:15px;word-break:break-all;display:flex;align-items:center;gap:8px"><span>{{ inspection.run.id }}</span><span class="status-dot" :class="inspection.run.status==='succeeded' ? 'status-dot--success' : inspection.run.status==='failed' ? 'status-dot--failed' : 'status-dot--running'" aria-hidden="true" /></h2>
          <p class="mono-text" style="margin-top:8px;display:flex;gap:8px;flex-wrap:wrap;align-items:center">
            <span class="pill" style="border-radius:999px">{{ inspection.workflow.status }}</span>
        ...
        <div style="display:flex;flex-direction:column;gap:8px;align-items:flex-end;position:relative">
      ...
      <section class="metric-grid metric-grid-detail">
        <article class="metric-card card-sheen" style="display:flex;align-items:center;gap:14px;min-height:88px;padding:16px 20px">
          <span style="display:grid;place-items:center;width:36px;height:36px;border-radius:10px;background:var(--brand-weak-bg);color:var(--brand-weak-text);border:1px solid var(--brand-weak-border)"><Clock3 :size="16" aria-hidden="true" /></span>
          <div>
            <p style="margin:0;font-size:11px;letter-spacing:0.08em;text-transform:uppercase;color:var(--text-muted);font-weight:600">Started</p>
            <strong style="font-size:14px">{{ formatDate(inspection.run.startedAt) }}</strong>
            <span>Created {{ formatDate(inspection.run.createdAt) }}</span>
          </div>
        </article>
        <article class="metric-card card-sheen" style="display:flex;align-items:center;gap:14px;min-height:88px;padding:16px 20px">
          <span style="display:grid;place-items:center;width:36px;height:36px;border-radius:10px;background:var(--neutral-bg);color:var(--neutral-text);border:1px solid var(--border)"><Activity :size="16" aria-hidden="true" /></span>
          <div>
            <p style="margin:0;font-size:11px;letter-spacing:0.08em;text-transform:uppercase;color:var(--text-muted);font-weight:600">Finished</p>
            <strong style="font-size:14px">{{ formatDate(inspection.run.finishedAt) }}</strong>
        ...
        <article class="metric-card card-sheen" style="display:flex;align-items:center;gap:14px;min-height:88px;padding:16px 20px">
          ...
          <div>
            <p style="margin:0;font-size:11px;letter-spacing:0.08em;text-transform:uppercase;color:var(--text-muted);font-weight:600">LLM usage</p>
            <strong style="font-size:14px">{{ formatNumber(inspection.usageTotals.totalTokens) }} tokens</strong>
            <span>{{ inspection.usageTotals.rounds }} rounds · {{ formatDuration(inspection.usageTotals.latencyMs) }}</span>
          </div>
        </article>
      </section>

      <section v-if="inspection.run.error" class="panel failure-panel" role="alert" style="border-radius:14px">
        <p class="eyebrow" style="color:var(--danger-text)">Run failure</p>
        <h2>{{ inspection.run.error.code }}</h2>
        <p>{{ inspection.run.error.message }}</p>
      </section>

      <!-- Execution timeline — premium -->
      <section class="panel panel--elevated table-panel">
        ...
        <div v-else class="timeline-premium">
          <div v-for="(step, idx) in inspection.steps" :key="step.id" class="timeline-premium-item">
            <div style="display:grid;justify-items:center;gap:6px">
              <span class="timeline-dot" :class="step.status==='succeeded' ? 'timeline-dot--success' : step.status==='failed' ? 'timeline-dot--failed' : step.status==='running' ? 'timeline-dot--running' : 'timeline-dot--queued'" aria-hidden="true">{{ step.status === 'succeeded' ? '✓' : step.status === 'failed' ? '✕' : step.status === 'running' ? '◷' : '•' }}</span>
              <span v-if="idx < inspection.steps.length - 1" style="width:1px;height:18px;background:var(--border);margin-top:2px" aria-hidden="true"></span>
            </div>
            <div style="min-width:0">
              ...
              <p v-if="step.error" style="margin:8px 0 0;padding:10px 12px;border-radius:10px;background:var(--danger-bg);border:1px solid #f3c6c1;color:#7a271a;font-size:12px"><strong>{{ step.error.code }}:</strong> {{ step.error.message }}</p>
              <details style="margin-top:10px">
                <summary style="cursor:pointer;color:var(--text-secondary);font-size:12px;font-weight:500;display:flex;align-items:center;gap:6px">Summaries (redacted) <span style="font-size:10px;color:var(--text-muted)">▾</span></summary>
                <div style="margin-top:8px;display:grid;gap:8px">
                  <div style="padding:10px;border:1px solid var(--border-subtle);border-radius:10px;background:var(--surface-raised)"><p style="margin:0 0 4px;color:var(--text-muted);font-size:11px;letter-spacing:0.06em;text-transform:uppercase">Input · {{ step.inputSummary.bytes }} bytes</p><p style="margin:0;font-family:var(--font-mono);font-size:12px;white-space:pre-wrap;word-break:break-word">{{ step.inputSummary.preview }}</p></div>
      ...
      <section class="detail-grid">
        <article class="panel panel--elevated table-panel card-sheen">
          ...
          <div v-else style="display:grid;gap:0">
            <div v-for="job in inspection.jobs" :key="job.id" style="display:flex;align-items:center;justify-content:space-between;gap:12px;padding:14px 0;border-top:1px solid var(--border-subtle)">
              <div style="min-width:0">
        ...
        <article class="panel panel--elevated table-panel card-sheen">
          ...
          <div v-if="inspection.llmUsage.length > 0" style="margin-top:14px;border-top:1px solid var(--border-subtle);padding-top:12px">
            <p style="margin:0 0 8px;color:var(--text-muted);font-size:12px;font-weight:600;letter-spacing:0.04em;text-transform:uppercase">LLM usage by round</p>
            <div style="display:grid;gap:8px">
              <div v-for="(u, i) in inspection.llmUsage" :key="i" style="display:flex;justify-content:space-between;gap:12px;padding:10px 12px;border:1px solid var(--border-subtle);border-radius:10px;background:var(--surface-raised);font-size:12px">
      ...
      <section class="panel panel--elevated context-panel">
        ...
        <div style="margin-top:16px;display:grid;gap:10px">
          <div style="padding:12px;border:1px solid var(--border-subtle);border-radius:10px;background:var(--surface-raised)">
