...
describe.skipIf(TEST_DATABASE_URL === undefined)('run inspection integration', () => {
  ...
  const seedRun = async (tenantId: string, source: string, ...stepKeys: string[]): Promise<string> => {
  ...
  it('orders step runs by start time and attributes llm usage + tools', async () => {
    ...
    await handle.db.insert(llmUsage).values([
      { tenantId: tenantA, runId, stepRunId: srA!.id, provider: 'claude', model: 'm', round: 1, inputTokens: 1, outputTokens: 2, totalTokens: 3, latencyMs: 10 },
      { tenantId: tenantA, runId, stepRunId: srB!.id, provider: 'claude', model: 'm', round: 1, inputTokens: 4, outputTokens: 5, totalTokens: 9, latencyMs: 20 },
      { tenantId: tenantA, runId, stepRunId: srB!.id, provider: 'claude', model: 'm', round: 2, inputTokens: 6, outputTokens: 7, totalTokens: 13, latencyMs: 30 },
    ]);
  ...
  it('summarizes and scrubs by default, and attaches scrubbed raw values in detail mode', async () => {
    ...
    expect(summary!.steps[0]!.outputSummary.preview).toContain('«redacted:secret-key»');
    expect(summary!.steps[0]!.outputSummary.preview).not.toContain('top-secret-value');

    const detailed = await repo(tenantA).getRun(runId, { detail: true });
    expect(JSON.stringify(detailed!.steps[0]!.output)).toContain('«redacted:secret-key»');
    expect(JSON.stringify(detailed!.steps[0]!.output)).not.toContain('top-secret-value');
  });
  ...
  it('returns null for another tenant’s run and for a nonexistent run', async () => {
    ...
    expect(await repo(tenantA).getRun('00000000-0000-0000-0000-000000000000')).toBeNull();
  });
  ...
  it('reports leased=true for a running job under a live lease', async () => {
    ...
    await handle.db
      .update(jobs)
      .set({ status: 'running', lockedBy: 'worker-1', leaseExpiresAt: new Date(Date.now() + 60_000) })
