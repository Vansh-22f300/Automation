  ...
  () => {
    ...
    beforeAll(async () => {
      ...
      const inserted = await handle.db
        .insert(tenants)
        .values([
          { name: "Connections Tenant A" },
          { name: "Connections Tenant B" },
        ])
    ...
    it("stores no plaintext and round-trips the secret via resolveForTool", async () => {
      const secret = {
        token: "xoxb-INTEGRATION-SECRET",
        refresh: "r-INTEGRATION",
      };
      ...
      const authorized = await repoFor(tenantA).resolveForTool({
        provider: "slack",
      });
    ...
    it("disable turns a connection off and refuses resolution afterwards", async () => {
      ...
      await expect(
        repoFor(tenantA).resolveForTool({ provider: "gmail" }),
      ).rejects.toBeInstanceOf(MissingConnectionError);
      await expect(
        repoFor(tenantA).resolveForTool({
          provider: "gmail",
    ...
    it("allows many active connections for the same (tenant, provider)", async () => {
      ...
      const [rowA] = await handle.db
        .select()
        .from(connections)
        .where(eq(connections.id, a.id));
      const [rowB] = await handle.db
        .select()
        .from(connections)
        .where(eq(connections.id, b.id));
      ...
      const resolvedA = await repoFor(tenantA).resolveForTool({
        provider: "slack",
        connectionId: a.id,
      });
      const resolvedB = await repoFor(tenantA).resolveForTool({
        provider: "slack",
        connectionId: b.id,
      });
    ...
    it("lists metadata newest first and pages without duplicates", async () => {
      const first = await repoFor(tenantA).create({
        provider: "linear",
        name: "alpha",
        credential: { token: "linear-A-SECRET" },
      });
      const second = await repoFor(tenantA).create({
        provider: "notion",
        name: "beta",
        credential: { token: "notion-A-SECRET" },
      });

      const sameTime = new Date("2099-01-02T00:00:00.000Z");
      await handle.db
        .update(connections)
      ...
      await handle.db
        .update(connections)
        .set({ createdAt: sameTime, updatedAt: sameTime })
        .where(eq(connections.id, second.id));

      const page1 = await repoFor(tenantA).listMetadataPage({ limit: 1 });
      expect(page1.items).toHaveLength(1);
      expect(page1.nextCursor).toBeTruthy();
      expect(JSON.stringify(page1.items[0])).not.toContain("linear-A-SECRET");
      expect(JSON.stringify(page1.items[0])).not.toContain("notion-A-SECRET");

      const page2 = await repoFor(tenantA).listMetadataPage(
      ...
      expect(page1.items[0]!.id > page2.items[0]!.id).toBe(true);
      expect(page2.items[0]!.id).not.toBe(page1.items[0]!.id);
    });

    it("rejects malformed cursors", async () => {
      await expect(
        repoFor(tenantA).listMetadataPage({ cursor: "not-base64-cursor" }),
    ...
    describe("tenant isolation", () => {
      ...
      it("A cannot update or disable B’s connection", async () => {
        expect(
          await repoFor(tenantA).updateMetadata(bConnId, { hijacked: true }),
        ).toBeNull();
        ...
        const [row] = await handle.db
          .select()
          .from(connections)
          .where(eq(connections.id, bConnId));
